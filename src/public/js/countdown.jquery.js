	...
	$.fn.countdown.begin = function (parent, settings) {
		...
		if (!settings.init) {
			...
			$.each(timespan, function (k, v) {
				...
				container = $('<div/>').attr('id', 'fiveld_'+k);

				// Create wrapper element
				wrapper = $('<div/>').addClass('fiveld_wrapper');

				// Create time element
				time = $('<span/>').addClass('fiveld_time').text(v < 10 ? '0' + v : v.toLocaleString());

				if (settings.labels) {
					// Create label element
					label = $('<span/>').addClass('fiveld_label').text((v === 1 ? $.fn.countdown.singularize(k) : k));

					// Add everything to container element
					container.append(wrapper.append(label).append(time));
				} else {
					container.append(wrapper.append(time));
				}

				// Add elements to parent element
				parent.append(container.addClass('fiveld_animated fiveld_rotateIn'));
			});
			...
			// Update each element
			$.each(timespan, function (k, v) {
				$('.fiveld_time', '#' + 'fiveld_'+k).text(v < 10 ? '0' + v : v.toLocaleString());
				$('.fiveld_label', '#' + 'fiveld_'+k).text((v === 1 ? $.fn.countdown.singularize(k) : k));
			});
		}
